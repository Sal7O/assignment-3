...
class fourByfour_Random_player: public Player {

protected:
    int dimension;
public:
    // Take a symbol and pass it to parent
    fourByfour_Random_player (char symbol, int dimension);
    // Generate a random move
    void get_move(int& x, int& y);

};

class fourByfour :public Board {
public:
    fourByfour();
    bool update_board (int x, int y, char mark) override;
    void display_board() override;
...
class Pyramic_Random_Player : public Player
{
protected:
    int dimension1;
    int dimension2;
public:
    // Take a symbol and pass it to parent
    Pyramic_Random_Player (char symbol, int dimension1 , int dimension2);
    // Generate a random move
    void get_move(int& x, int& y);
};
void X_O_Runner();
